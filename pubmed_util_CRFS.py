# Author: Forrest Hooton; Adapted by Zita Martins


import requests #  allows to send HTTP requests using Python
from lxml import etree #library for processing XML and HTML in the Python language; lxml.etree tries to follow established APIs wherever possible
import pandas as pd
import numpy as np
import math
import time
...
def search_pubmed(search_terms):
	...
	url = construct_url(search_terms, 'search') # used to retrieve the appropriate URL path and filter_string to make the request to the APIC

	xml = __safe_urlopen__(url)

	root = etree.fromstring(xml) # implements a simple and efficient API for parsing and creating XML data
...
def construct_url(url_input, query_type, num_results = 1000000):
	...
	if query_type == 'search':
		...
		############################### Cap the years CHANGE DATE HERE! #########################################################
		date_type_url = 'datetype=pdat'
		min_year_url = 'mindate={2010}'
		max_year_url = 'maxdate={2023}'

		# Cap the number of results
		results_num_url = '&retmax=' + str(num_results)

		return base_url + term_url + date_type_url + term_url + min_year_url + term_url + max_year_url + results_num_url
